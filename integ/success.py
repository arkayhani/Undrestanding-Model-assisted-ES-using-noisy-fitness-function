...
for j in b:  
    ...
    #h.append(integrate.quad(fun2, 0,np.inf,args=(i,b))[0])
            #h.append((integrate.quad(fun3, -np.inf,np.inf,args=(i,j))[0]))#*(1.0/(np.sqrt(2*pi)*i)))
            h1.append((integrate.quad(fun2anew, -np.inf,np.inf,args=(i,j))[0])*(1.0/(np.sqrt(2*pi))))
            h4.append(cdf(-np.power(i,2)/(2*np.sqrt(np.power(i,2)+np.power(j,2)))))
            #h2.append((integrate.quad(fun32, -np.inf,np.inf,args=(i,j))[0])/(integrate.quad(fun3, -np.inf,np.inf,args=(i,j))[0]))
        prints.append(h1)
        prints2.append(h4)
        #prints.append(h1)
        #prints.append(h2)
...
plt.figure(4)
plt.plot(a,prints[0],"-b",a,prints[1],"-b",a,prints[2],"-b",a,prints[3],"-b",a,prints2[1],"-b")
print(h)
print(h1)
